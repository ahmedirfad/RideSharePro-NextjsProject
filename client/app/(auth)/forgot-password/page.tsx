      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <Link href="/login" className="text-sm font-medium text-gray-600 hover:text-blue-600 transition px-4 py-2">
            Remembered? <span className="font-semibold text-blue-600">Sign In →</span>
          </Link>
        </div>
      </nav>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="space-y-8">
              ...
              <h1 className="font-bold text-[clamp(36px,5vw,52px)] text-gray-900 leading-tight tracking-tight">
                Recover your<br />
                <span className="bg-gradient-to-r from-blue-600 to-cyan-500 bg-clip-text text-transparent">account access</span>
              </h1>

              <p className="text-gray-500 text-lg leading-relaxed max-w-md">
                Follow the quick verification steps to securely reset your password.
              </p>

              {/* Feature Cards */}
              <div className="space-y-4 pt-4">
                {[
                  { icon: Shield, title: 'Secure Recovery', desc: 'Protected by OTP verification to your email', color: 'text-blue-600', bg: 'bg-blue-50' },
                  { icon: Star, title: 'Instant Reset', desc: 'No complex waiting times or delay loops', color: 'text-amber-600', bg: 'bg-amber-50' },
                  { icon: Car, title: 'Ride Safely', desc: 'Keeping your commuter profile safe at all times', color: 'text-green-600', bg: 'bg-green-50' },
                ].map((feature, i) => (
